...
def summarize(node):
    if node.layout:
        layout = {
            ...
            "padding_box": {
                "position": (
                    node.layout.absolute_padding_box_left,
                    node.layout.absolute_padding_box_top,
                ),
                "size": (node.layout.padding_box_width, node.layout.padding_box_height),
            },
            "border_box": {
                "position": (
                    node.layout.absolute_border_box_left,
                    node.layout.absolute_border_box_top,
                ),
...
class LayoutTestCase(TestCase):
    ...
    def layout_node(self, node):
        root = ExampleNode(style=CSS(display=BLOCK), children=[node])
    ...
    def _assertLayout(self, actual, expected, output, depth=0):
        ...
        try:
            self.assertAlmostEqual(
                ...
                actual.layout.content_width,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["content"]["size"][1],
                actual.layout.content_height,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["content"]["position"][0],
                actual.layout.absolute_content_left,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["content"]["position"][1],
                actual.layout.absolute_content_top,
                delta=tolerance,
            )
        ...
        try:
            self.assertAlmostEqual(
                expected["padding_box"]["size"][0],
                actual.layout.padding_box_width,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["padding_box"]["size"][1],
                actual.layout.padding_box_height,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["padding_box"]["position"][0],
                actual.layout.absolute_padding_box_left,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["padding_box"]["position"][1],
                actual.layout.absolute_padding_box_top,
                delta=tolerance,
            )
        except AssertionError:
            found_problem = True
        ...
        try:
            self.assertAlmostEqual(
                expected["border_box"]["size"][0],
                actual.layout.border_box_width,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["border_box"]["size"][1],
                actual.layout.border_box_height,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["border_box"]["position"][0],
                actual.layout.absolute_border_box_left,
                delta=tolerance,
            )
            self.assertAlmostEqual(
                expected["border_box"]["position"][1],
                actual.layout.absolute_border_box_top,
                delta=tolerance,
            )
        except AssertionError:
            found_problem = True
    ...
    def find_tests(cls, test_filename, group):
        ...
        def make_test(test_dir, filename):
            ...
            def test_method(self):
                ...
                self.assertLayout(root, reference, "\n" + "\n".join(extra))
